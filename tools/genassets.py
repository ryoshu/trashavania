...
# horizontally for left-facing. Jimothy has a spinal deformity: he walks
# bent over, back arched high, head carried low and forward. Reading the
# silhouette left->right: ringed tail raised behind (cols 0-4), arched
# hump, then the head DOWN AND FORWARD at the right edge -- black mask
# band with white eyes, white muzzle, black nose at the leading edge.
# ===========================================================================
...
# Crouch -- 16x16 raccoon scrunch: the hunch, but MORE so. Tail hugs the
# back, chin nearly on the ground.
JIM_CROUCH = """\
................
................
..KKK...........
...
# Swipe -- head/hump rows of JIM_TOP, then the near arm thrusts forward
# past the snout; pair with the slash effect sprite. Rooted stance.
JIM_SWIPE = """\
..KKK...........
.KKKKK..........
.KGGGK..........
...
# Hurt -- knocked back, eyes squeezed shut, legs flailing forward
JIM_HURT = JIM_TOP.replace(".KGGGGGGKKWKKWKK", ".KGGGGGGKKKWKWKK") + """\
KGGGGGGGGGGGKK..
.KGGGGGGGGGGGK..
.KKGGGGGGGGGKK..
..KGGGGGGGGGK...
...
# ---------------------------------------------------------------------------
# Count Dumpula sprites (palette 2: black/gray/yellow). The dumpster body is
# background tiles; only the lid, eyes and thrown garbage are sprites.

# Lid: 32x8 (4x1 tiles), a slamming plate with two handles.
# Built programmatically so the 32-char row widths are exact.
LID = "\n".join([
    "K" * 32,
    "K" + "G" * 30 + "K",
    "KGWW" + "G" * 27 + "K",
...
SPRITES = [
    ...
    add_sprite("LID", LID, 32, 8),
    add_sprite("EYE", EYE, 8, 8),
    add_sprite("GARB", GARB, 8, 8),
]
...
# --- atmosphere set: all collision NONE (background dressing) except
# where noted; added for the "Nintendo quality" polish pass ---

metatile("gravestone", 0, 0, """\
................
................
.....22222......
....2333332.....
...
# ---------------------------------------------------------------------------
# Title-screen portrait: the Jimothy idle frame upscaled 2x into background
# tiles (32x48 px, 4x6 tiles). Drawn on the title in BG palette 0 (grays),
# so K->dark blue outline, G->gray fur, W->light gray face.

TITLE_JIM_W, TITLE_JIM_H = 4, 6
_tj_src = parse_art(JIM_IDLE, SPRITE_CHARMAP, 16, 24)
_tj_grid = [[_tj_src[y // 2][x // 2] for x in range(32)] for y in range(48)]
TITLE_JIM_FIRST = bg_bank.add_grid(_tj_grid, TITLE_JIM_W, TITLE_JIM_H,
                                   dedupe=False)
...
################
""",
         spawns=[("SP_BAT", 6, 10), ("SP_BAG", 3, 12), ("SP_SNACK", 7, 10),
                 ("SP_BIGSNACK", 1, 8)],
         player_start=(1, 13),
         exits={'door': 1}),

    Room("Recycling Crypt", """\
................
................
BBBBBBBBBBBBBBBB
..c.....c.....c.
...
BBBBBBBBBBBBBBBB
""",
         spawns=[("SP_CAT", 11, 12), ("SP_BAT", 3, 9),
                 ("SP_CANDLE", 4, 9), ("SP_WEAPON_CAP", 5, 11),
                 ("SP_BURRITO", 13, 12), ("SP_BIGSNACK", 11, 5)],
         player_start=(1, 13),
         exits={'door': 2}),
...
BBBBBBBBBBBBBBBB
""",
         spawns=[("SP_BAT", 8, 8), ("SP_GNOME", 10, 4),
                 ("SP_GOLD", 1, 12), ("SP_SNACK", 2, 6),
                 ("SP_CANDLE", 6, 5)],
         player_start=(6, 14),
         exits={'door': 3}),

    Room("Chapel of Questionable Leftovers", """\
................
................
BBBBBBBBBBBBBBBB
..c.....c.....c.
...
BBBBBBBBBBBBBBBB
""",
         spawns=[("SP_GNOME", 12, 12), ("SP_CAT", 5, 12),
                 ("SP_BAT", 4, 9), ("SP_WEAPON_TOMATO", 2, 10),
                 ("SP_BURRITO", 13, 10), ("SP_BAG", 10, 12)],
         player_start=(0, 13),
         exits={'door': 4}),

    Room("The Moonlit Dumpster", """\
................
................
BB..........BBBB
Bc...........c..
...
def emit():
    ...
    c.append("")
    c.append("/* Bulk data lives in PRG bank 0 ($8000-$BFFF, selected once at")
    c.append("   reset and never switched -- see crt0.s). */")
    c.append('#pragma rodata-name (push, "PRGBANK0")')
    c.append("")
    ...
    # title portrait
    h.append("")
    h.append("#define TILE_TITLE_JIM %d" % TITLE_JIM_FIRST)
    h.append("#define TITLE_JIM_W %d" % TITLE_JIM_W)
    h.append("#define TITLE_JIM_H %d" % TITLE_JIM_H)
